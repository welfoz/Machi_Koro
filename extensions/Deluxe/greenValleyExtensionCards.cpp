...
void MovingCompany::activation(Player &p) {
    auto p2=Controller::getInstance().getProxy()->getInterface()->selectOnePlayerDifferentFromTheCurrentOne(&p);
    auto givenCardPtr=Controller::getInstance().getProxy()->getInterface()->selectOneEstablishmentCardFromPlayer(&p,"Which card do you want to give to "+p2->getUsername());
...
void DemolitionCompany::activation(Player &p) {
    auto monumentPtr= Controller::getInstance().getProxy()->getInterface()->selectMonumentCardFromCurrentPlayer(&p,"Which monument do you want to demolish ?");
...
void RenovationCompany::activation(Player &p) {
    auto renovatedCardPtr=Controller::getInstance().getProxy()->getInterface()->selectOneCardOwnedByAnyPlayer("Which non-major establishment do you want to renovate ?");
...
void InternationalExhibitHall::activation(Player &p) {
    if (Controller::getInstance().getProxy()->getInterface()->confirmationDialog("Do you want to activate another of your non-Major establishment card ?","Yes","No")){
        auto cardPtr= Controller::getInstance().getProxy()->getInterface()->selectOneEstablishmentCardFromPlayer(&p,"Which non-Major establishment card do you want to activate ?");
        cardPtr->activation(p);
        p.removeEstablishment(this);
        Controller::getInstance().getProxy()->getInterface()->printBasicMessage(cardPtr->getName() + " returned to the market.");
    }
